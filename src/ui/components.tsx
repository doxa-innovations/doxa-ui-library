// src/pages/ComponentsPage.tsx (React SPA)
...
function CodeBlock({ code }: { code: string }) {
    return (
        <div className="relative isolate overflow-hidden rounded-2xl border border-white/10 bg-black backdrop-blur-xl shadow-[0_0_0_1px_rgba(255,255,255,0.06)]">
            ...
            <pre className="relative z-0 m-0 max-w-full overflow-x-auto p-4 text-xs text-white">
    ...
    return (
        <div className="mt-4 rounded-xl border border-white/10 bg-white/5">
            <button
                ...
                aria-expanded={open}
                onClick={() => setOpen((v) => !v)}
                className="w-full flex items-center justify-between gap-3 px-3 py-2 text-sm text-zinc-200 hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-white/30 rounded-t-xl"
            >
                <span>{open ? 'Hide code' : label}</span>
                <svg className={cx('h-4 w-4 transition-transform', open && 'rotate-180')} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            ...
            <div className={cx('grid transition-all duration-300 ease-in-out overflow-hidden', open ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0')}>
...
/* Sidebar demo that renders your layout in a sandbox and offers raw HTML to copy */
function SidebarDemo() {
    const [open, setOpen] = useState(false);

    return (
        <div className="rounded-2xl border border-purple-400/30 bg-purple-500/15 p-4 backdrop-blur-xl shadow-[0_0_0_1px_rgba(168,85,247,0.25)]">
            <div className="mb-3 flex items-center justify-between">
                <span className="text-sm text-zinc-300/80">Default Sidebar Layout</span>
                <CopyButton text={DEFAULT_SIDEBAR_SNIPPET} />
            </div>

            {/* PREVIEW SANDBOX (absolute/h-full so it doesn't hijack the page) */}
            <div className="relative h-[640px] w-full overflow-hidden rounded-xl bg-white dark:bg-[#0c1222] ring-1 ring-white/10">
                {/* Mobile toggle — we keep your classes & data-attrs for fidelity, but wire React onClick */}
                <button
                ...
                {/* Sidebar (SANDBOXED): absolute + h-full. The copy-snippet uses fixed/h-screen. */}
                <aside
                    id="default-sidebar"
                    aria-label="Sidebar"
                    className={cx(
                        'absolute top-0 left-0 z-40 w-64 h-full transition-transform -translate-x-full sm:translate-x-0',
                        open && 'translate-x-0'
                    )}
                >
                ...
                {/* Content sandbox (mirrors your grid/boxes) */}
                <div className="absolute inset-0 p-4 sm:ml-64">
            ...
            {/* Raw HTML for copy (your exact snippet) */}
            <Collapsible label="View code">
                <CodeBlock code={DEFAULT_SIDEBAR_SNIPPET} />
            </Collapsible>
        </div>
    );
}

const DEFAULT_SIDEBAR_SNIPPET = `<button data-drawer-target="default-sidebar" data-drawer-toggle="default-sidebar" aria-controls="default-sidebar" type="button" class="inline-flex items-center p-2 mt-2 ms-3 text-sm text-gray-500 rounded-lg sm:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600">
    ...
    return (
        <main className="min-h-screen bg-[#0b1220] text-white">
            <div className="mx-auto max-w-6xl px-4 py-10">
                ...
                <p className="text-zinc-300/80 mb-8">Your provided layout is rendered below (sandboxed) and available as exact HTML via Copy.</p>
